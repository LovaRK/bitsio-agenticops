...
const FALLBACK_WINS: QuickWin[] = [
  { rank: 1, category: "Monitor Utilization", action: "Review sourcetypes with zero searches in 90 days", sourcetype: "all_low_activity_sources", estimated_impact_usd: 0, details: "Baseline hygiene sweep for inactive telemetry paths." },
  { rank: 2, category: "Improve Detection Coverage", action: "Increase MITRE ATT&CK coverage for network sourcetypes", sourcetype: "network_*", estimated_impact_usd: 0, details: "Map uncategorized network data to ATT&CK-aligned detection content." },
  { rank: 3, category: "Validate Retention", action: "Confirm retention policies match data tier classification", sourcetype: "tiered_indices", estimated_impact_usd: 0, details: "Align retention horizons with Critical/Important vs Nice/Wasteful classification." },
];
...
export function QuickWinsTable({ wins }: QuickWinsTableProps) {
  ...
  const rows = wins.length === 0 ? FALLBACK_WINS : wins;
    ...
    <div className="rounded-xl border border-outline-variant/10 bg-surface-container p-5 mb-6">
      ...
      {wins.length === 0 && (
        <p className="mb-3 text-sm italic text-on-surface-variant">
          No high-impact actions detected - monitoring suggestions below.
        </p>
      )}
