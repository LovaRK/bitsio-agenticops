  ...
  return (
    <div className="space-y-4" data-testid="decision-gate">
      ...
      <div className="mt-4 flex gap-3">
        <button
          className="flex-1 rounded-xl bg-secondary text-on-secondary px-4 py-3 text-sm font-bold transition-all active:scale-95 hover:scale-[1.02]"
          onClick={() => onDecision("approved")}
        >
          <span className="material-symbols-outlined text-sm mr-2 inline-block">check_circle</span>
          Approve
        </button>
        <button
          className="flex-1 rounded-xl bg-error text-on-error px-4 py-3 text-sm font-bold transition-all active:scale-95 hover:scale-[1.02]"
          onClick={() => onDecision("rejected")}
        >
          <span className="material-symbols-outlined text-sm mr-2 inline-block">cancel</span>
          Reject
        </button>
      </div>
      {status === "done" && (
        <p className="mt-2 text-xs text-secondary font-medium">✓ Approval decision sent.</p>
      )}
    </div>
  );
}
