"""Shared helpers for assembling incident traces from Splunk rows."""
...
def extract_text(row: dict[str, Any], key: str, *, default: str) -> str:
...
def extract_int(value: Any, *, default: int) -> int:
    try:
        if isinstance(value, list):
            value = value[0] if value else default
...
def extract_time(value: Any) -> str:
    if isinstance(value, list):
...
def normalize_incident_id(ref: str) -> str:
    token = ref.strip()
...
def build_decision_trace_from_rows(
    *,
    ...
    first = rows[0]
    severity = normalize_severity(extract_text(first, "severity", default="medium"))
    title = extract_text(first, "title", default=f"Incident {incident_id}")
    status = "pending_approval" if severity == "high" else "triaging"
    timestamp = extract_time(first.get("_time") or first.get("time"))

    host_counts = Counter(extract_text(row, "host", default="unknown-host") for row in rows)
    probable_cause = ", ".join(
    ...
    return {
        ...
        "evidence_refs": build_evidence_refs(
            incident_id=incident_id,
            source_index=source_index,
            splunk_web_base_url=splunk_web_base_url,
        ),
        "missing_evidence": [],
        "node_runs": [
            {
                ...
                "tool_calls": [
                    {
                        ...
                        "provider": f"{model_provider}/{runtime_mode}",
